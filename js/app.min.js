...
const animationsInn2 = () => {
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      const img1 = document.querySelector(".innovations__img_1");
      if (entry.isIntersecting) {
        img1.classList.add("innovations__img_anim");
        return;
      }
      img1.classList.remove("innovations__img_anim");
    });
  });

  observer.observe(document.querySelector(".effects__target"));
};

window.addEventListener("scroll", function () {
  const scrollPosition = window.scrollY;
  const parallaxBlock = document.querySelector(".innovations__img_1");
  parallaxBlock.style.transform = `translateY(${10 + -scrollPosition * 0.015}%`;
});
